...
import {ScrollView, FlatList, SafeAreaView, StyleSheet, Text, View, Button, TextInput} from 'react-native';
import {getAllEvents, searchUsersByUsername, getUserRegisteredEvents, unregisterEvent, registerEvent} from "../api/ProEventoAPI";
...
const renderItem = (item, navigation, myAccount, toggleOverlay) => {
  return (
  <Item event={item} navigation={navigation} myAccount={myAccount} toggleOverlay={toggleOverlay}/>
)};

const renderUser = (item, navigation, myAccount) => {
  return (
  <UserCard user={item} navigation={navigation} myAccount={myAccount}/>
)};
    ...
    unregisterEvent(
      {
        ...
        //    }
        // });
        setCurrentUserRegister(currentUserRegister.filter((item)=>{
          return item.id !== event.id
        }));
        console.log("successfully unregister this event");
      }
    );
  }
  ...
  const onRegisterEvent = (event, user) => {
    registerEvent(
      ...
      ,()=>{
        ...
        console.log("successfully register this event");
      }
    );
  }
  ...
  else
  {
    return (
        <View style={styles.container}>
          ...
          <FlatList
          data={shownList}
          renderItem={item => renderItem(item.item, navigation, myAccount, toggleOverlay)}
          ...
          {shownUserList !== []? 
            <FlatList
              ...
              keyExtractor={item => item.id.toString()}
              ListHeaderComponent={()=><Text>{shownUserList.length!=0?"Found users":""}</Text>}
            />
          :null}
